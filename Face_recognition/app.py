    ...
    def submit_data(self):
        if self.file_path:
            ...
            if employee_id and first_name and last_name and nfc_data:
                ...
                image = QPixmap(self.file_path).toImage().convertToFormat(QImage.Format.Format_RGB888)
                image_np = np.array(image)
                rgb_image = image_np[:, :, :3]  # Drop the alpha channel if present

                if rgb_image.shape[2] == 3 and rgb_image.shape[0] > 1:
                ...
                if rgb_image is not None:
                    ...
                    QMessageBox.warning(self, "Image Conversion Error", "Failed to convert image to RGB format.")
            else:
...
class EmployeeManagementWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Initially hide buttons until the "Employee" table is selected
    ...
    def load_log_data(self):
        ...
        try:
            ...
            for row_idx, row in enumerate(result):
                self.table.setItem(row_idx, 0, QTableWidgetItem(str(row.employee_id)))
                self.table.setItem(row_idx, 1, QTableWidgetItem(row.room_number))
